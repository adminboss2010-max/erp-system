...
## استيراد الأصناف بالجملة من إكسل ✅

ميزة استيراد أصناف بالجملة من ملف إكسل، لتسريع إدخال بيانات الأصناف لشركة جديدة بدل الإدخال اليدوي واحد واحد من `items.html`. النسخة الحالية موسّعة عن أول نسخة (راجع تاريخ التعديل تحت) لتغطية ربط مجموعة الوحدات (UoM) وقفل ثغرة تكرار الكود/الباركود.
...
### القالب — ملف إكسل ثنائي الشيت، 13 عمود (`downloadTemplate`)

**شيت "تعليمات"**: تعليمات نصية للمستخدم — من ضمنها توضيح صريح إن حقل مجموعة الوحدات اختياري، وإن اسم مجموعة غير موجودة بيتجاهل بدون ما يوقف الاستيراد.

**شيت "الأصناف"**: صف رؤوس أعمدة + صف مثال واحد. الأعمدة الـ13 بالترتيب: `الكود *`, `الاسم *` (إلزاميان)، `التصنيف`, `الماركة`, `الوحدة الأساسية`, **`مجموعة الوحدات (اختياري)`** 🆕, `الباركود`, `سعر التكلفة`, `سعر البيع`, `الكمية الابتدائية`, `الحد الأدنى للمخزون`, `الحد الأقصى للمخزون`, `الوصف`.

### القراءة والاستيراد (`parseFile` + `importItems`)

- **`parseFile(file)`**: بتقرأ الملف المرفوع عبر `FileReader` + `XLSX.read`، بتدوّر تحديدًا على شيت اسمه `"الأصناف"` (أو أول شيت لو الاسم مختلف)، وبتفلتر أي صف من غير كود أو اسم. الحقل الجديد `مجموعة الوحدات (اختياري)` بيتقرأ كـ `uomGroupName`.
- **`importItems(companyId, items)`**: بتستورد كل صنف فعليًا فى جدول `items` عبر `supabaseClient.from('items').insert(...)`:
  ...
  - **🆕 ربط مجموعة الوحدات بالاسم — تجاهل آمن، مش عرقلة**: بتحمّل كل `uom_groups` الموجودة مسبقًا للشركة فى كاش (`uomGroupCache`)، وبتربط الصنف بمجموعته لو الاسم مطابق تمامًا. **لا يتم إنشاء مجموعات وحدات جديدة من الإكسل** (خلافًا للتصنيفات) — دي عمداً بتتعرّف يدويًا بس عبر `create_uom_group` لأنها بتحتاج تعريف معاملات تحويل دقيقة (`conversion_factor`) مش مجرد اسم. لو الاسم المكتوب فى الإكسل مش مطابق لأي مجموعة موجودة، الصنف **بيتسجل عادي بدون مجموعة وحدات** (`uom_group_id = null`) مع تسجيل تحذير واضح فى تقرير الأخطاء (`"مجموعة الوحدات ... غير موجودة، تم تجاهلها"`) — **الاستيراد مبيتوقفش ولا الصف بيتلغي**.
  - بترجع تقرير مفصّل: `{ success, failed, errors: [] }` — كل صف فاشل (أو تحذير تجاهل مجموعة وحدات) بيتسجل بسببه، بدل ما العملية كلها تتوقف عند أول خطأ.

**ملاحظة معمارية**: الاستيراد بيحصل عبر PostgREST مباشرة (`insert` صف بصف) مش عبر Edge Function أو RPC ذرية واحدة — يعني لو حصل فشل فى نص عملية استيراد ملف فيه مئات الأصناف، الأصناف اللي نجحت قبل الفشل بتفضل متسجلة (مش rollback كامل زي دوال المعاملات التجارية). ده قرار مقبول هنا لأن الاستيراد عملية إعداد أولي (setup) مش معاملة محاسبية.

### ⚠️ ثغرة اتكتشفت واتصلحت: عدم وجود قيد فريد على كود/باركود الصنف

**المشكلة**: قبل هذا الإصلاح، مفيش أي قيد على مستوى قاعدة البيانات يمنع تكرار `items.code` أو `items.barcode` لنفس الشركة. يعني الاستيراد بالجملة (أو حتى الإدخال اليدوي من `items.html`) كان ممكن يُنشئ أكتر من صنف بنفس الكود أو نفس الباركود بدون ما حد ينتبه، وده بيكسر أي منطق مستقبلي بيفترض إن الكود معرّف فريد للصنف (مثلاً البحث بالباركود عند نقطة البيع).

**الإصلاح** (مُتحقق منه مباشرة من قاعدة البيانات الحية بتاريخ 2026-07-24):
- **`UNIQUE (company_id, code)`** — قيد `items_company_code_unique` كان موجود بالفعل، تم التأكيد عليه:
  ```sql
  ALTER TABLE items ADD CONSTRAINT items_company_code_unique UNIQUE (company_id, code);
  ```
- **`items_company_barcode_unique`** — **partial unique index** جديد على `(company_id, barcode)`، بشرط `barcode IS NOT NULL` (عشان أصناف من غير باركود متعددة تفضل مسموحة، من غير ما نمنع أكتر من صنف بباركود `NULL`):
  ```sql
  CREATE UNIQUE INDEX items_company_barcode_unique
  ON items(company_id, barcode) WHERE barcode IS NOT NULL;
  ```
  تم إنشاؤه وتأكيد وجوده فعليًا عبر `pg_indexes` بعد التنفيذ.

بكده أي محاولة استيراد أو إدخال يدوي لصنف بكود أو باركود مكرر لنفس الشركة **هترفض على مستوى قاعدة البيانات نفسها** (مش بس على مستوى الواجهة)، وهتظهر كخطأ واضح فى `results.errors` أثناء الاستيراد بالجملة بدل ما تتسجل بصمت.

### الاختبار الفعلي (End-to-End) — استيراد ناجح مع ربط تصنيف ومجموعة وحدات صحيحين ✅

**تم التحقق مباشرة من جدول `items`** (join مع `item_categories` و`uom_groups`) بعد استيراد فعلي من ملف إكسل مبني على القالب: أصناف مستوردة زي `أرز بسمتي 3.5 كجم` (كود `5`) و`أرز مصرى 3.5 كجم` (كود `6`) اترتبطا صح بـ:
- **تصنيف**: `مواد غذائية` (اتنشأ تلقائيًا أو اترتبط بتصنيف موجود، عبر `categoryCache`) ✅
- **مجموعة وحدات**: `كرتون-شد-قطعة` (اترتبطا بيها بالاسم عبر `uomGroupCache`، بدون ما تتنشئ مجموعة جديدة) ✅
- باركود فريد لكل صنف (`6281234567820` و`6281236567891`) بدون أي تعارض مع القيد الجديد ✅

**دلالة النتيجة**: مسار الاستيراد الكامل (قراءة الملف → إنشاء تصنيف تلقائي → ربط مجموعة وحدات موجودة بالاسم → إدراج الصنف فى قاعدة البيانات) بيشتغل صح من طرف لطرف، وقيود منع التكرار الجديدة (كود + باركود) موجودة وفعّالة بدون ما تكسر مسار الاستيراد الطبيعي للأصناف الفريدة.

---

## المجموعة الحرجة من العمليات (Critical Operations Group) — مكتملة ✅

كل العمليات المحاسبية الأساسية اللي أي نظام ERP لازم يغطيها قبل ما يُعتبر صالح للاستخدام الفعلي بقت مبنية، منشورة، ومربوطة بواجهة `app-new.html` — بنفس الأنماط المعمارية المثبتة طول المشروع (RPC ذرية + Edge Function رقيقة + قيود محاسبية تلقائية + RLS).

| العملية | Edge Function | دالة RPC | الحالة |
|---|---|---|---|
| فاتورة مبيعات | `post-sale` | `post_sale_transaction` | ✅ منشورة، تسميتها فى الواجهة "فاتورة مبيعات" (مش "تسجيل بيع") |
| فاتورة مشتريات | `post-purchase` | `post_purchase_transaction` | ✅ منشورة، تسميتها فى الواجهة "فاتورة مشتريات" (مش "تسجيل شراء") |
| سند قبض (دفعة من عميل) | `post-payment` | `post_payment_transaction` | ✅ منشورة، بحقول موسّعة (`payment_method`, `payment_date`, `reference`, `notes`) |
| مرتجع مبيعات | `post-return` | `post_return_transaction` | ✅ منشورة، بحماية من الإفراط فى الإرجاع |
| مرتجع مشتريات | `post-purchase-return` | `post_purchase_return_transaction` | ✅ منشورة (دالة RPC جديدة كليًا، مبنية بتاريخ 2026-07-25) |

### تصحيح المسميات المحاسبية فى الواجهة (`app-new.html`)

قرار عمل متعمد: التسميات فى الواجهة اتصححت من صياغة "إجرائية" عامة لصياغة محاسبية دقيقة، فى كل الأماكن التالية (رابط Sidebar، أزرار "نظرة عامة السريعة"، عنوان الشاشة، النص الفرعي، رسائل الـ toast، عنوان جدول السجل):
- "تسجيل بيع" / "تسجيل عملية بيع" / "تم تسجيل البيع" → **"فاتورة مبيعات"** / **"تم إصدار فاتورة المبيعات رقم ..."**
- "تسجيل شراء" / "تسجيل عملية شراء" / "تم تسجيل الشراء" → **"فاتورة مشتريات"** / **"تم إصدار فاتورة المشتريات رقم ..."**

السبب: "فاتورة" مصطلح محاسبي دقيق (مستند له رقم تسلسلي وأثر محاسبي مزدوج القيد)، بعكس "تسجيل" اللي بيوحي بعملية إدخال بيانات عادية بدون دلالة محاسبية.

### مرتجع المبيعات (`post-return`) — حماية من الإفراط فى الإرجاع

دالة `post_return_transaction` بتفحص، لكل صنف فى المرتجع:
```sql
-- الكمية المباعة أصلاً فى الفاتورة المرجع إليها
select coalesce(sum(qty), 0) into v_original_qty ... where doc_no = p_ref_doc_no and type = 'sale';
-- الكمية اللي اترجعت فعلاً قبل كده لنفس الفاتورة
select coalesce(sum(qty), 0) into v_already_returned ... where ref_doc_no = p_ref_doc_no and type = 'return';

if (v_already_returned + v_qty) > v_original_qty then
  raise exception 'الكمية المرتجعة تتجاوز المباع أصلًا — المباع: %، المرتجع سابقًا: %، المطلوب الآن: %', ...;
end if;
```
يعني مينفعش ترجع كمية أكبر من اللي اتباعت أصلاً، ولا ترجع نفس الكمية مرتين عبر مرتجعات متعددة لنفس الفاتورة. الحماية دي بتشمل UoM (تحويل الوحدة قبل المقارنة) والقيود المحاسبية العكسية (عكس الإيراد عبر حساب `4100` مردودات المبيعات، عكس الضريبة، عكس تكلفة البضاعة المباعة).

### مرتجع المشتريات (`post-purchase-return`) — دالة جديدة كليًا، بنفس منطق الحماية

**السياق**: مكانتش موجودة من الأساس فى المرحلة 3 (كانت بس `post-sale`/`post-return`/`post-payment`/`post-purchase`). اتبنت من الصفر بتاريخ 2026-07-25 بنفس نمط `post_return_transaction` بالظبط لكن معكوسة (مرتجع للمورد بدل من العميل):
- نفس حماية الإفراط فى الإرجاع (منقارنة بالمشترى أصلاً بدل المباع أصلاً)
- **فحص إضافي غير موجود فى مرتجع المبيعات**: التأكد إن المخزون الحالي كافي لإرجاع الكمية للمورد (`v_current_stock < v_base_qty → raise exception`) — منطقي لأن مرتجع المشتريات بيقلل المخزون (بيرجعه للمورد)، بعكس مرتجع المبيعات اللي بيزود المخزون
- قيد محاسبي: مدين `2000`/`1000` (رد قيمة للمورد) ← دائن `1200` (تخفيض المخزون) + دائن `1300` (عكس ضريبة مدخلات لو فيه)
- نوع معاملة جديد `purchase_return` (اتضاف لـ `transactions_type_check` من قبل، راجع قسم "نظام المخزون الاحترافي")

**النشر**: `supabase functions new post-purchase-return` ثم `supabase functions deploy post-purchase-return` من داخل `D:\erp-system-main`.

**الواجهة**: رابط Sidebar "مرتجع مشتريات" + `renderNewPurchaseReturn`/`loadPurchaseReturnableItems`/`submitPurchaseReturnAction`/`loadPurchaseReturnsLog` فى `app-new.html`، بنفس نمط دوال مرتجع المبيعات بالظبط، بس بـ`PurchaseReturnsClient` (موجودة مسبقًا فى `supabase-client.js`) بدل `ReturnsClient`.

### ⚠️ درسان تقنيان مهمان (لأي جلسة مستقبلية)

**1. خطر لصق كود جديد فوق كود قديم فى ملفات Edge Functions بدون مسح المحتوى أولاً**: حصل فعليًا أثناء تحديث `post-payment/index.ts` — الكود الجديد (بدعم `payment_method`/`payment_date`/`reference`/`notes`) اتلصق **فى آخر الملف** بدل ما يستبدل المحتوى القديم، فبقى الملف فيه نسختين من الدالة (القديمة بدون CORS، والجديدة بيها) لصقتين فوق بعض — كود مكسور بـ`Deno.serve` مكرر مرتين. **الحل المتبع**: قبل أي "الصق الكود ده فى الملف"، لازم "مسح المحتوى بالكامل الأول" (استخدام `Write` بمحتوى كامل جديد، مش `Edit`/append) بدل الافتراض إن اللصق هيستبدل تلقائيًا. **الدرس**: أي طلب "افتح الملف والصق فيه X" لازم يُفهم كـ"استبدال كامل"، ويتحقق من محتوى الملف بعد الكتابة للتأكد من عدم وجود تكرار.

**2. قطع/بتر أكواد SQL طويلة عند اللصق فى Supabase Dashboard (SQL Editor)**: لصق دوال PL/pgSQL طويلة (100+ سطر، زي `post_purchase_return_transaction`) مباشرة فى SQL Editor بالمتصفح عرضة للقطع أو الفشل الصامت لأسباب متعلقة بحدود اللصق/الـ textarea فى الواجهة. **الحل المُعتمد فعليًا فى هذا المشروع**: تنفيذ الكود عبر Claude Code مباشرة — إنشاء ملف `.sql` مؤقت فى الريبو المحلي (`Write` tool) ثم تشغيله بالكامل دفعة واحدة عبر:
```
supabase db query --file <filename>.sql --linked
```
(ملحوظة: الأمر الصحيح فى نسخة الـ CLI الحالية هو `db query --file`، مش `db execute` — `execute` مش subcommand موجود أصلاً). هذا المسار تم تجربته فعليًا لبناء `post_purchase_return_transaction` ونجح من غير أي قطع، مع تحقق مباشر بعدها من `pg_proc` للتأكد من وجود الدالة فعليًا.

---

## ⚠️ Cash Sale Bug — ثغرة محاسبية اتكتشفت وانصلحت ✅

### الوصف

أي عملية **نقدية** (مش آجلة — `is_credit = false`) فى البيع، الشراء، أو مرتجع الشراء كانت بتسجّل **`debit = 0` و`credit = 0` معًا** فى صف الصنف بجدول `transactions`. النتيجة العملية: أي فاتورة نقدية كانت بتظهر بمبلغ **صفر** فى الطباعة (`printDocument`) وفى أي تقرير بيعتمد على `debit + credit` كقيمة السطر (زي `DocumentsClient.getFullDocument` و`DocumentsClient2.listAll` المستخدمَين فى "سجل المستندات")، **رغم إن القيد المحاسبى نفسه (`journal_entries`/`journal_entry_lines`) كان صحيحًا ومتوازنًا 100%** طول الوقت — لأنه محسوب من متغير SQL منفصل تمامًا (`v_grand_total`) مش من عمودي `debit`/`credit` دول. يعني الميزان المحاسبى (Trial Balance) ما كانش هيكشف المشكلة دي أبدًا، لأنها مش خلل فى القيد نفسه، بل فى بيانات عرض/تقارير منفصلة.

### السبب الجذري

الدوال الثلاث كانت بتملأ **عمود واحد بس** شرطيًا وتسيب التانى قيمة ثابتة `0` بدل ما تملأ الاتنين شرطيًا (عكس بعض):

| الدالة | كان (خطأ) | بقى (صح) |
|---|---|---|
| `post_sale_transaction` | `debit = amount لو آجل وإلا 0`, **`credit = 0` دايمًا** | `debit = amount لو آجل وإلا 0`, `credit = amount لو نقدي وإلا 0` |
| `post_purchase_transaction` | `debit = amount لو آجل وإلا 0`, **`credit = 0` دايمًا** | نفس نمط `post_sale_transaction` بالظبط |
| `post_purchase_return_transaction` | **`debit = 0` دايمًا**, `credit = amount لو آجل وإلا 0` | `debit = amount لو نقدي وإلا 0`, `credit = amount لو آجل وإلا 0` |

**ملاحظة مهمة**: `post_return_transaction` (مرتجع المبيعات) كان **سليم من الأساس** — كان بيستخدم النمط الصحيح (`case when p_is_credit then 0 else amount end` / `case when p_is_credit then amount else 0 end`) من وقت بنائه فى المرحلة 4. ده هو النمط المرجعي اللي اتبنى عليه إصلاح الدالتين التانيتين (`post_purchase_transaction`/`post_purchase_return_transaction`، اللي اتصلحوا ليطابقوا نفس منطق الاتجاه المعكوس المستخدم فى `post_return_transaction` كونهم "مرتجعات").

### كيف اتكتشفت

أثناء بناء نظام الطباعة الاحترافي الجديد (`printDocument()` + `DocumentsClient.getFullDocument`)، ولاحظت إن حسابات `lineTotal = debit + credit` بترجع صفر لفواتير بيع نقدية حقيقية كانت موجودة بالفعل (`doc_no = 5, 6, 8, 9`). تحققت مباشرة من قاعدة البيانات وأكدت الصفر فعليًا (مش خطأ عرض)، وبعدين راجعت تعريف الدالة الحية وأكدت السبب الجذري.

### الإصلاح والاختبار الفعلي (End-to-End)

الثلاث دوال اتصلحت عبر `CREATE OR REPLACE FUNCTION` (ملفات `fix_sale_cash_credit.sql`, `fix_purchase_cash_credit.sql`, `fix_purchase_return_cash_credit.sql`، بنفس أسلوب `db query --file` الموثّق فوق)، وكل واحدة اتختبرت فعليًا بنداء RPC حقيقي لعملية **نقدية**، مع تحقق مباشر من قيم `debit`/`credit` الفعلية بعدها:

| الدالة | الاختبار | النتيجة قبل الإصلاح (متوقعة) | النتيجة بعد الإصلاح (فعلية) |
|---|---|---|---|
| `post_sale_transaction` | بيع نقدي، `qty=1`, `unit_price=15` | `debit=0, credit=0` | `debit=0, credit=15` ✅ |
| `post_purchase_transaction` | شراء نقدي، `qty=2`, `unit_cost=5` | `debit=0, credit=0` | `debit=0, credit=10` ✅ |
| `post_purchase_return_transaction` | مرتجع شراء نقدي، `qty=1`, `unit_cost=5` | `debit=0, credit=0` | `debit=5, credit=0` ✅ |

**الميزان المحاسبى اتفحص بعد كل إصلاح وفضل متوازن تمامًا** (`trial_balance`: `total_debit = total_credit`, `difference = 0`) على كل الشركات، بما فيها الشركة اللي اتعمل عليها الاختبار — إثبات عملي إن الإصلاح لمس عرض/تقارير البيانات بس، ومامسّش صحة القيد المحاسبى الأساسي اللي كان سليم من الأصل.

### ⚠️ ملاحظة مهمة جدًا — بيانات قديمة لسه فيها القيمة الخاطئة

**الفواتير النقدية اللي اتسجلت قبل هذا الإصلاح** (على الأقل `doc_no = 5, 6, 8, 9` من اختبارات UoM السابقة، وأي فاتورة/مرتجع نقدي تانى اتسجل قبل تاريخ هذا الإصلاح 2026-07-26) **لسه فيها `debit=0` و`credit=0` فعليًا فى قاعدة البيانات** — الإصلاح بينطبق بس على العمليات الجديدة من دلوقتي، **مش تراجعي (retroactive)**. القيد المحاسبى بتاعها (`journal_entries`) صحيح ومتوازن، لكن لو حد حاول يطبع أو يعرض تقرير لأي فاتورة نقدية قديمة من دول، هيشوف مبلغ صفر.

**قرار مؤجل يحتاج اتخاذه لاحقًا**: هل نعمل `UPDATE` تراجعي على السجلات القديمة دي (بحساب القيمة الصحيحة من `journal_entry_lines` أو من `unit_price × qty` المخزنين فى نفس الصف) وتصحيحها، ولا نسيبها زي ما هي باعتبارها بيانات اختبار قديمة (كل الشركات المتأثرة حاليًا هي شركات اختبار داخلية، مفيش بيانات عميل حقيقي متأثرة وقت كتابة هذا التحديث)؟ لسه القرار ده مفتوح.
